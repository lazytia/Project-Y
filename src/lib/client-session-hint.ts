...
/** Runs synchronously in layout HTML — must stay free of imports. */
export const BOOT_SPLASH_EARLY_DISMISS_SCRIPT = `(function(){var b=document.getElementById("boot-splash");if(!b)return;var s=document.getElementById("server-app-shell");var f=document.getElementById("static-chrome-fallback");var hint=false;try{hint=localStorage.getItem("y.authed")==="1";}catch(e){}if(document.cookie.indexOf("y_sess=1")!==-1)hint=true;if(s||hint){b.classList.add("bootSplashHidden");if(!s&&f&&hint)f.removeAttribute("hidden");}})();`;
